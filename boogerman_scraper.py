...
"""
Scraper implementation for boogerman.nl website.
"""
...
class BoogermanScraper(BaseScraper):
    """Scraper for boogerman.nl website."""
    
    def __init__(self):
        """Initialize the Boogerman scraper."""
        super().__init__(
            name="boogerman",
            base_url="https://www.boogerman.nl"
        )
        self.logger = logging.getLogger("scraper.boogerman")
    
    def get_listings(self, filters: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        """
        Get property listings from boogerman.nl.
        ...
        for element in property_elements:
            try:
                ...
                listing = {
                    'id': f"boogerman_{property_url.split('/')[-1]}",
                    'address': address,
                    'city': city,
                    'price': price,
                    'area': area,
        ...
        self.logger.info(f"Found {len(listings)} listings on boogerman.nl")
        return listings
    ...
    def get_property_details(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            details['id'] = f"boogerman_{property_id}"
