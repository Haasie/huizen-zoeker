...
"""
Scraper implementation for voornemakelaars.nl website.
"""
...
class VoorneMakelaarsScraper(BaseScraper):
    """Scraper for voornemakelaars.nl website."""
    
    def __init__(self):
        """Initialize the Voorne Makelaars scraper."""
        super().__init__(
            name="voornemakelaars",
            base_url="https://www.voornemakelaars.nl"
        )
        self.logger = logging.getLogger("scraper.voornemakelaars")
    
    def get_listings(self, filters: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        """
        Get property listings from voornemakelaars.nl.
        ...
        for element in property_elements:
            try:
                ...
                listing = {
                    'id': f"voornemakelaars_{property_url.split('/')[-1]}",
                    'address': address,
                    'city': city,
                    'price': price,
                    'area': area,
        ...
        self.logger.info(f"Found {len(listings)} listings on voornemakelaars.nl")
        return listings
    ...
    def get_property_details(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            details['id'] = f"voornemakelaars_{property_id}"
