...
"""
Scraper implementation for woonvoorn.nl website.
"""
...
class WoonVoornScraper(BaseScraper):
    """Scraper for woonvoorn.nl website."""
    
    def __init__(self):
        """Initialize the Woon Voorn scraper."""
        super().__init__(
            name="woonvoorn",
            base_url="https://www.woonvoorn.nl"
        )
        self.logger = logging.getLogger("scraper.woonvoorn")
    
    def get_listings(self, filters: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        """
        Get property listings from woonvoorn.nl.
        ...
        for element in property_elements:
            try:
                ...
                listing = {
                    'id': f"woonvoorn_{property_url.split('/')[-1]}",
                    'address': address,
                    'city': city,
                    'price': price,
                    'area': area,
        ...
        self.logger.info(f"Found {len(listings)} listings on woonvoorn.nl")
        return listings
    ...
    def get_property_details(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            details['id'] = f"woonvoorn_{property_id}"
